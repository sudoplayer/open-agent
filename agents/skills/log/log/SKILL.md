---
name: log
description: 可指定底数的对数运算技能。计算 log_base(value) 并向用户确认结果。
---
...
3. 否则（`ok` 为 `true`），取 `result` 作为计算结果，调用 `ask_user_question` 向用户确认。问题中包含底数、真数和计算结果，选项如 `["确认", "拒绝", "结果有误需要修改"]`
4. 根据用户回复决定返回内容
...
- **用户指出参数错误** → 用正确的底数和真数重新运行脚本，再次确认
- **用户拒绝且无具体修正** → 向主编排报告"用户拒绝了 log_{base}({value}) 的结果"
