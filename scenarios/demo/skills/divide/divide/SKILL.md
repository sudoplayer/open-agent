...
- `python3 scenarios/demo/skills/divide/scripts/divide_numbers.py --a <a> --b <b>` — 纯计算，返回 JSON `{ok, result, message}`。除数为 0 时返回 `ok: false`，`message` 包含错误信息
...
1. 运行脚本得到结果：
   ```bash
   python3 scenarios/demo/skills/divide/scripts/divide_numbers.py --a <a> --b <b>
   ```
   解析 JSON 输出。
