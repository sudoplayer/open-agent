---
name: subtract
description: 两数减法运算技能。计算 a - b 并向用户确认结果。
---
...
- `python3 scenarios/demo/skills/subtract/scripts/subtract_numbers.py --a <a> --b <b>` — 纯计算，返回 JSON `{ok, result, message}`
...
1. 运行脚本得到计算结果：
   ```bash
   python3 scenarios/demo/skills/subtract/scripts/subtract_numbers.py --a <a> --b <b>
   ```
...
- **用户给出修正值**（如"应该是3""改成3"）→ 返回用户修正后的值，不再重试
